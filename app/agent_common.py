...
GREETING_TEMPLATES = {
    "en": {
        ...
        "has_Appt":   "Hello {first_name}, I see you have an appointment coming up on {appointment_date} at {appointment_time}. Would you like to reschedule your appointment, or could I help you with something else?",
    },
    "es": {
...
def build_dynamic_greeting_and_next(agent, lang: str):
    ...
    # Normalize fields from both sources
    first = (customer or {}).get("first") or cd.get("first_name", "") or ""
    last  = (customer or {}).get("last")  or cd.get("last_name", "")  or ""

    year  = (vehicle or {}).get("year")  or cd.get("car_year", "")  or ""
    make  = (vehicle or {}).get("make")  or cd.get("car_make", "")  or ""
    model = (vehicle or {}).get("model") or cd.get("car_model", "") or ""

    appointment_id   = (vehicle or {}).get("appointment_id")   or cd.get("appointment_id")   or ""
    appointment_date = (vehicle or {}).get("appointment_date") or cd.get("appointment_date") or ""
    appointment_time = (vehicle or {}).get("appointment_time") or cd.get("appointment_time") or ""

    # Robust flags
    appt_id_str = str(appointment_id).strip()
    has_appt    = (appt_id_str.isdigit() and int(appt_id_str) > 0) or (bool(appointment_date) and bool(appointment_time))
    has_name    = bool(first)
    has_vehicle = bool(year and model)

    # Safe formatter: missing keys become ""
    ...
    data = _SafeDict(str, {
        ...
        "name": (first + (" " + last if last else "")) if (first or last) else "",
        # vehicle
        "year": year, "make": make, "model": model,
        "vehicle": " ".join([v for v in (year, make, model) if v]),
        # appt
    ...
    tmpl_all = GREETING_TEMPLATES.get(lang, GREETING_TEMPLATES["en"])

    def pick_template(*candidates):
        for k in candidates:
            if k in tmpl_all and tmpl_all[k]:
                return tmpl_all[k]
        # last-resort fallback keys
        return tmpl_all.get("generic") or tmpl_all.get("no_name") or "Hello."

    if has_appt:
        text = pick_template("has_appt", "has_Appt", "appointment_found").format_map(data)
        next_state = "get service"  # or "cancel reschedule" if that’s your flow
    elif has_name and has_vehicle:
        text = pick_template("full", "name_and_vehicle").format_map(data)
        next_state = "get service"
    elif has_name:
        text = pick_template("no_vehicle", "name_only", "name").format_map(data)
        next_state = "get car"
    else:
